...
function Player.create(posX, yOffset, posZ, rotX, rotY, isAbsolute)
  ...
  if posX ~= nil and yOffset ~= nil and posZ ~= nil then
    if isAbsolute then
      Player.moveToAbsolute(posX, yOffset, posZ)
    else
...
function Player.moveTo(x, yOffset, z)
  ...
  local y = terrainHeight + yOffset + Player.height * 0.5
...
function Player.moveToAbsolute(x, y, z)
  setTranslation(Player.rootNode, x, y + Player.height * 0.5, z)
  Player.lastXPos = x
  Player.lastYPos = y + Player.height * 0.5
  Player.lastZPos = z
end
