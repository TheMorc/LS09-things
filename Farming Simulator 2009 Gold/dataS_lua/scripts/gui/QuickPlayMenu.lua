...
function QuickPlayMenu:new(backgroundOverlay)
  ...
  xPos = xPos + instance.buttonWidth + instance.backPlayButtonSpacing
  instance:addButton(OverlayButton:new(Overlay:new("delete_button", "dataS/menu/delete_button" .. g_languageSuffix .. ".png", buttonSpacingSide, instance.backPlayButtonSpacing, instance.buttonWidth, instance.buttonHeight), QuickPlayMenu.deleteAreYouSureDialog, instance))
  instance:addButton(OverlayButton:new(Overlay:new("reset_vehicles_button", "dataS/menu/reset_vehicles_button" .. g_languageSuffix .. ".png", buttonSpacingSide + instance.buttonWidth + instance.backPlayButtonSpacing, instance.backPlayButtonSpacing, instance.largeButtonWidth, instance.buttonHeight), QuickPlayMenu.resetVehiclesAreYouSureDialog, instance))
  ...
  if g_isDemo then
    instance.demoLockedOverlay = Overlay:new("demoLockedOverlay", "dataS/menu/demo_locked" .. g_languageSuffix .. ".png", 0, 0, instance.imageSize * 0.75, instance.imageSize)
  end
  ...
  instance.dialogGUIText = 1
  table.insert(instance.dialogOverlays, Overlay:new("dialog_overlay", "dataS/menu/dialog_bg.png", 0.3, 0.5, 0.4, 0.26666666666666666))
  instance:addDialogButton(OverlayButton:new(Overlay:new("no_button", "dataS/menu/no_button" .. g_languageSuffix .. ".png", 0.5 - 0.5 * instance.buttonWidth + 0.095, 0.53, instance.buttonWidth, instance.buttonHeight), OnQuickPlayMenuDiffBack))
  instance:addDialogButton(OverlayButton:new(Overlay:new("yes_button", "dataS/menu/yes_button" .. g_languageSuffix .. ".png", 0.5 - 0.5 * instance.buttonWidth - 0.095, 0.53, instance.buttonWidth, instance.buttonHeight), QuickPlayMenu.OnQuestionYes, instance))
  return instance
end
function QuickPlayMenu:mouseEvent(posX, posY, isDown, isUp, button)
  ...
  elseif self.difficultyGUIActive then
...
function QuickPlayMenu:render()
  if not self.difficultyGUIActive and not self.dialogGUIActive then
  ...
  elseif self.dialogGUIActive then
    self.backgroundOverlay:render()
    ...
    setTextBold(true)
    setTextColor(1, 1, 1, 1)
    if self.dialogGUIText == 1 then
      renderText(0.495, 0.7, 0.038, g_i18n:getText("YouWantToDeleteThat"))
    elseif self.dialogGUIText == 2 then
      renderText(0.496, 0.7, 0.038, g_i18n:getText("YouWantToResetVehicles"))
    end
    setTextBold(false)
    setTextAlignment(RenderText.ALIGN_LEFT)
  end
...
function QuickPlayMenu:addDialogButton(overlayButton)
...
function QuickPlayMenu:deleteAreYouSureDialog()
  local savegame = self.savegames[self.selectedIndex]
  if savegame.valid and not g_isDemo then
    self.dialogGUIActive = true
    self.dialogGUIText = 1
  end
end
function QuickPlayMenu:resetVehiclesAreYouSureDialog()
  local savegame = self.savegames[self.selectedIndex]
  if savegame.valid and not g_isDemo then
    self.dialogGUIActive = true
    self.dialogGUIText = 2
  end
end
function QuickPlayMenu:OnQuestionYes()
  if self.dialogGUIText == 1 then
...
function QuickPlayMenu:loadStatsFromXML(baseXMLName, savegame)
  ...
  savegame.stats.farmSiloFruitAmount = {}
  local siloString = getXMLString(g_savegameXML, baseXMLName .. "#farmSiloFruitAmount")
  if siloString ~= nil then
  ...
  savegame.stats.fruitPrices = {}
  local pricesString = getXMLString(g_savegameXML, baseXMLName .. "#fruitPrices")
  if pricesString ~= nil then
  ...
  savegame.stats.yesterdaysFruitPrices = {}
  local pricesString = getXMLString(g_savegameXML, baseXMLName .. "#yesterdaysFruitPrices")
  if pricesString ~= nil then
    local prices = Utils.splitString(" ", pricesString)
    for k, v in pairs(prices) do
  ...
  local moneyStr = getXMLString(g_savegameXML, baseXMLName .. "#money")
  local money = Utils.getNoNil(tonumber(moneyStr), QuickPlayMenu.defaultMoney)
  ...
  if savegame.stats.money ~= QuickPlayMenu.defaultMoney then
    ...
    local hash3 = self:getMoneyHash(savegame.stats.money + 1)
    local hash4 = self:getMoneyHash(savegame.stats.money - 1)
    if hash == nil or hash ~= hash2 and hash ~= hash3 and hash ~= hash4 then
      savegame.stats.money = QuickPlayMenu.defaultMoney
...
function QuickPlayMenu:saveStatsToXML(baseXMLName, savegame)
  ...
  local moneyStr = tostring(savegame.stats.money)
  setXMLString(g_savegameXML, baseXMLName .. "#money", moneyStr)
...
function QuickPlayMenu:getStatsFromMission(savegame)
  ...
  savegame.stats.saveDate = os.date(g_i18n:getText("shortDate"))
end
function QuickPlayMenu:getMoneyHash(money)
  local hash = getMD5(string.format("%d", (math.floor(tonumber(tostring(money))))))
  local ret = ""
  for i = 1, hash:len() do
