...
source("dataS/scripts/gui/GUIBase.lua")
...
function init()
  ...
  delete(xmlFile)
  print("Farming Simulator 2009 Gold")
  print("  Version: 1.0 RC6 Patch 1")
  print("  Language: " .. g_languageShort)
  local inputBindingPathTemplate = getAppBasePath() .. "data/inputBindingDefault.xml"
  ...
  for state, filename in pairs(cursorFilenames) do
    overlays[state] = Overlay:new(filename, "data/menu/" .. filename .. ".png", 0, 0, 0.03, 0.04)
  end
...
function loadMod(modName, modDir, modFile)
  local validForSource = getIsValidModDir(modName)
  function printInvalidModName(modDir)
    print("Error: invalid mod name '" .. modDir .. "'! Characters allowed: (_, A-Z, a-z, 0-9). The first character must not be a digit")
  end
  ...
  if modDescVersion ~= 1 and modDescVersion ~= 2 then
  ...
  I18N.initModI18N(g_i18n, modName, modEnv.g_i18n)
  local i = 0
  while true do
  ...
  while true do
    ...
    if not validForSource then
      printInvalidModName(modName)
      return
    end
