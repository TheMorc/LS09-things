...
function BaseMission:new(customMt)
  ...
  instance.hudHelpBaseWidth = 0.425
...
function BaseMission:loadMap(name)
  ...
  local timeGrowthScale = 1 + (g_settingsTimeScale - 1) * -0.30000000000000004 / 59
...
function BaseMission:addItemToSave(i3dFilename, item)
  table.insert(self.itemsToSave, {node = item, i3dFilename = i3dFilename})
end
...
function BaseMission:draw()
  if self.isRunning then
    ...
    if g_settingsHelpText then
      ...
      if 1 <= table.getn(renderTextsLeft) then
        self.hudHelpBaseOverlay:render()
      end
      for i = 1, num do
        local left = renderTextsLeft[i]
        local right = renderTextsRight[i]
        renderText(0.02, (4 - i) * 0.03 + (self.hudHelpBasePosY + 0.03), 0.025, left)
        renderText(0.221, (4 - i) * 0.03 + (self.hudHelpBasePosY + 0.03), 0.025, right)
      end
    end
...
function BaseMission:onLeaveVehicle()
  if not self.trailerIsTipping then
    ...
    dx, dy, dz = localDirectionToWorld(self.controlledVehicle.exitPoint, 0, 0, 1)
    Player.onEnter()
