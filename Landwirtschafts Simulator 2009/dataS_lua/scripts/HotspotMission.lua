...
function HotspotMission:keyEvent(unicode, sym, modifier, isDown)
  if sym == Input.KEY_9 and isDown then
    self.state = HotspotMission.STATE_FINISHED
    self.endTime = 999
    HotspotMission:superClass().finishMission(self, 999)
  end
...
function HotspotMission:draw()
  ...
  if self.isRunning then
    ...
    setTextColor(1, 1, 1, 1)
    setTextBold(true)
    renderText(self.hudMissionBasePosX + 0.005, 0.926, 0.04, g_i18n:getText("hotspotMissionGoal") .. tostring(self.numTriggers - self.touchedTriggerCount))
    setTextBold(false)
