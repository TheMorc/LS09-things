...
function Vehicle:new(configFile, baseDirectory, positionX, offsetY, positionZ, yRot, specializations, customMt)
  ...
  while true do
    ...
    if wheel.repr == nil then
      ...
      wheel.lateralStiffness = Utils.getNoNil(getXMLFloat(xmlFile, wheelnamei .. "#lateralStiffness"), 3.5)
  ...
  if self.isActive then
    ...
    for k, implement in pairs(self.attachedImplements) do
      ...
      if jointDesc.allowsJointLimitMovement then
        ...
        if not implement.object.attacherJoint.fixedRotation then
          newRotLimit = Utils.getMovedLimitedValues(implement.jointRotLimit, jointDesc.maxRotLimit, {
            0,
            0,
            0
        ...
        local newTransLimit = Utils.getMovedLimitedValues(implement.jointTransLimit, jointDesc.maxTransLimit, {
          0,
          0,
          0
        }, 3, jointDesc.moveTime, dt, not jointDesc.moveDown)
...
function Vehicle:handleDetachAttachableEvent()
  if self.selectedImplement ~= 0 then
    ...
    if not attachable:handleDetachAttachableEvent() then
