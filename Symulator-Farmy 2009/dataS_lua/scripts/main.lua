...
source("dataS/scripts/gui/SplashScreen.lua")
...
function init()
  ...
  local startSavegameIndex = getXMLInt(startOptionsXML, "startOptions.careerStart#savegame")
  local splashScreenDisabled = Utils.getNoNil(getXMLBool(startOptionsXML, "startOptions.splashScreen#disabled"), false)
  if splashScreenDisabled then
    g_showSplashScreen = false
  end
  ...
  if not g_isDemo and startSavegameActive and startSavegameIndex ~= nil then
    OnMainMenuQuickPlay()
    gameMenuSystem.quickPlayMenu.selectedIndex = Utils.clamp(startSavegameIndex, 1, table.getn(gameMenuSystem.quickPlayMenu.savegames))
